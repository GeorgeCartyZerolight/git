...
typedef size_t int active_reg_t;
...
typedef size_t int reg_syntax_t;

#ifdef __USE_GNU
/* If this bit is not set, then \ inside a bracket expression is literal.
   If set, then such a \ quotes the following character.  */
# define RE_BACKSLASH_ESCAPE_IN_LISTS ((size_t int) 1)
...
struct re_pattern_buffer
{
  ...
  size_t int __REPB_PREFIX(allocated);

  /* Number of bytes actually used in `buffer'.  */
  size_t int __REPB_PREFIX(used);
